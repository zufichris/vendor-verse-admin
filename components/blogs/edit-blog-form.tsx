...
export function EditBlogForm({
  ...
  const jsonContent = useMemo(() => {
    if (!blog?.jsonContent) return null;
    try {
      console.log(typeof blog.jsonContent)
      return typeof blog.jsonContent === 'string' ? JSON.parse(blog.jsonContent) : blog.jsonContent;
    } catch {
      console.warn("Failed to parse blog JSON");
      return null;
    }
  }, [blog.jsonContent]);

  const editorConfig = useMemo(() => getDefaultEditorConfig(), [jsonContent])
  ...
  const form = useForm<BlogFormValues>({
    ...
    defaultValues: {
      ...
      tags: blog?.tags?.length ? blog.tags.join(',') : "",
      jsonContent: blog?.jsonContent || "",
      htmlContent: blog?.htmlContent || "",
    },
  })

  const imageDisplayUrl = useWatch({
    control: form.control,
    name: "mainImageUrl",
  });

  useEffect(() => {
  const url = imageDisplayUrl?.trim();
  if (!url || uploadingMainImage || url === blog.mainImageUrl) return;

  const handler = setTimeout(() => {
    const img = new Image();
    img.onload = () => form.clearErrors("mainImageUrl");
    img.onerror = () =>
  ...
  async function onSubmit(values: BlogFormValues) {
    ...
    try {
        ...
        const data = await Api.put<Blog>(`/blogs/${blog.id}`, {...values, tags: values.tags.split(",").map(tag => tag.trim())});
  ...
  const handleMainImageChange:ChangeEventHandler<HTMLInputElement> = async(e)=>{
    ...
    if (!file.type.startsWith('image')) {
        toast("Error", {
            description: "Please only upload images"
        })
        return;
    }
              ...
              <FormControl>
                <div>
                    <div className="flex items-center justify-center gap-2">
                        ...
                        <Button type="button"  variant={'outline'} className="flex items-center justify-center cursor-pointer relative">
                            <Label htmlFor="uploadMainImageEdit">
                            <Upload className="mr-2 h-4 w-4" />
                            <Input type="file" accept="image/*" name="uploadMainImageEdit" id="uploadMainImageEdit" className="absolute w-0 h-0 -z-50" onChange={handleMainImageChange} />
                            </Label>
                        </Button>
                    </div>
