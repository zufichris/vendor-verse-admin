...
import { Selection as SelectionExt } from "@tiptap/extensions"
...
import type { Editor, JSONContent, NodeWithPos, UseEditorOptions } from "@tiptap/react"
import StarterKit from "@tiptap/starter-kit"
import { uploadFile } from "./actions/files-manager"
...
export const handleImageUpload = async (
  ...
  while (true) {
    ...
    onProgress?.({ progress: (uploadedSize / totalSize) * 100 });
  }
  ...
  try {
    return await uploadFile(formData, abortSignal) as unknown as string
  } catch (error) {
    console.log(error)
    return '';
  }
...
export function getDefaultEditorConfig(defaultContent?: JSONContent): UseEditorOptions {
  return {
